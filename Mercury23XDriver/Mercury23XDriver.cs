...
namespace Drivers.Mercury23XDriver
{
    ...
    public class Mercury23XDriver : CMeter, IMeter
    {
        ...
        public bool ReadSoftwareVersion(ref string softwareVersion)
        {
            if (ReadVersionMeter())
            {
                softwareVersion = m_version.ToString();
                return true;
            }
            else
            {
        ...
        public bool ReadCurrentValues(ref Values values)
        {
            ...
            // CRC16
            CalcCRC(ref m_cmd, m_length_cmd);
            for (int i = 0; i < m_crc.Length; i++)
            {
            ...
            if (m_vport.WriteReadData(FindPacketSignature, m_cmd, ref in_buffer, m_length_cmd, -1) > 0)
            {
                ...
                if (find_header)
                {
                    ...
                    // проверка CRC
                    CalcCRC(ref in_buffer, Convert.ToUInt16(packet_length - 2));

                    if (in_buffer[packet_length - 2] == m_crc[0] && in_buffer[packet_length - 1] == m_crc[1])
                    {
                        // проверка ID
        ...
        private Mercury23XMeterTypes? meterType = null;
                    ...
                    if (find_header)
                    {
                        ...
                        if (in_buffer[packet_length - 2] == m_crc[0] && in_buffer[packet_length - 1] == m_crc[1])
                        {
                            ...
                            try
                            {
                                ...
                                if (Enum.IsDefined(typeof(Mercury23XMeterTypes), (int)swVersion))
                                {
                                    this.meterType = (Mercury23XMeterTypes)(int)swVersion;
                                    softwareVersion = Enum.GetName(typeof(Mercury23XMeterTypes), (int)swVersion);
                                }
                                else
                                {
        ...
        public bool WriteValue(double value, int channel = 1, bool isDouble = true)
        {
            ...
            byte[] cmdArray = cmdList.ToArray();
            CalcCRC(ref cmdArray, Convert.ToUInt16(cmdList.Count));
            cmdList.AddRange(m_crc);
